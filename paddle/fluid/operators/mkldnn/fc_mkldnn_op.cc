...
GetOutputScales(const ExecutionContext& ctx) {
...
std::shared_ptr<dnnl::memory> AcquireMemoryWithReorderAndAttrs(
  ...
  auto reorder_p =
      std::make_shared<dnnl::reorder>(*user_memory_p, *target_memory_p, attrs);
...
std::shared_ptr<dnnl::memory> AcquireBiasMemoryWithReorder(
  ...
  if (phi::funcs::is_int8<T_w>() == false) {
    ...
    if (!memory_p) {
      ...
      memory_p =
          this->AcquireMemoryWithReorderAndAttrs(user_md,
...
std::shared_ptr<dnnl::memory> AcquireWeightsMemoryWithReorder(
  ...
  if (!memory_p) {
    ...
    auto user_md = dnnl::memory::desc(
        weights_dims, OneDNNGetDataType<float>(), dnnl::memory::format_tag::io);
